...
class Rubic_Cube():
    def __init__(self):
        ...
        # Set up scene for mouse interaction
        scene.bind('mousedown', self.mouse_down)
        scene.bind('mousemove', self.mouse_move)
        scene.bind('mouseup', self.mouse_up)
        scene.forward = vector(0, -1, -2)  # Adjust initial camera position
    ...
    def mouse_down(self, event):
        self.dragging = True
        self.last_pos = vector(event.pos)
        
    def mouse_move(self, event):
        if not self.dragging:
            return
            
        current_pos = vector(event.pos)
        ...
        scene.forward = rotate(scene.forward, angle=-diff.x * sensitivity, axis=vector(0, 1, 0))
        scene.forward = rotate(scene.forward, angle=diff.y * sensitivity, axis=cross(scene.forward, vector(0, 1, 0)))
